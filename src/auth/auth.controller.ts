...
export class AuthController {
  ...
  @Get('verify')
  @HttpCode(HttpStatus.OK)
  async verifyToken(@Headers('authorization') authorization?: string | string[]) {
    try {
      ...
      if (authHeader && typeof authHeader === 'string') {
        const preview = authHeader.length > 48 ? `${authHeader.slice(0, 48)}...` : authHeader;
      ...
      if (!uuid) return { success: false, error: 'Missing required parameter: uuid' };
